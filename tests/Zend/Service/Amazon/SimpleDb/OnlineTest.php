...
class Zend_Service_Amazon_SimpleDb_OnlineTest extends PHPUnit_Framework_TestCase
{
    ...
    public function request($method, $args = [])
    {
        ...
        for ($try = 1; $try <= $this->_testWaitRetries; $try++) {
            try {
                $this->_wait();
                $response = call_user_func_array([$this->_amazon, $method], $args);
                break;
    ...
    public function testBatchPutAttributes() {
        ...
        try {
            ...
            $result = $this->request('getAttributes', [$domainName, $itemName2, $attributeName4]);
            $this->assertEquals([$newAttributeValue4, $attributeValue4, $attributeValue5], $result[$attributeName4]->getValues());
