...
class Zend_Service_WindowsAzure_Storage_Queue extends Zend_Service_WindowsAzure_Storage
{
	...
	public function putMessage($queueName = '', $message = '', $ttl = null)
	{
		...
		if (strlen((string) $message) > self::MAX_MESSAGE_SIZE) {
			// require_once 'Zend/Service/WindowsAzure/Exception.php';
	...
	public function deleteMessage($queueName, Zend_Service_WindowsAzure_Storage_QueueMessage $message)
	{
		...
		// Perform request
		$response = $this->_performRequest($queueName . '/messages/' . $message->MessageId, '?popreceipt=' . urlencode((string) $message->PopReceipt), Zend_Http_Client::DELETE);
		if (!$response->isSuccessful()) {
			// require_once 'Zend/Service/WindowsAzure/Exception.php';
			throw new Zend_Service_WindowsAzure_Exception($this->_getErrorMessage($response, 'Resource could not be accessed.'));
		}
    ...
    public static function isValidQueueName($queueName = '')
    {
        ...
        if (strpos((string) $queueName, '--') !== false) {
            return false;
        }

        if (strtolower((string) $queueName) != $queueName) {
            return false;
        }

        if (strlen((string) $queueName) < 3 || strlen((string) $queueName) > 63) {
            return false;
        }

        if (substr((string) $queueName, -1) == '-') {
            return false;
        }
