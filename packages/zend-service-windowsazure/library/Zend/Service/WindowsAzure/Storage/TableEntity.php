...
class Zend_Service_WindowsAzure_Storage_TableEntity
{
    ...
    public function getAzureValues()
    {
        ...
        foreach ($accessors as $accessor) {
            ...
            } else if ($accessor->EntityType == 'ReflectionMethod' && substr((string) strtolower((string) $accessor->EntityAccessor), 0, 3) == 'get') {
    ...
    public function setAzureValues($values = array(), $throwOnError = false)
    {
        ...
        foreach ($accessors as $accessor) {
            if (isset($values[$accessor->AzurePropertyName])) {
                ...
                } else if ($accessor->EntityType == 'ReflectionMethod' && substr((string) strtolower((string) $accessor->EntityAccessor), 0, 3) == 'set') {
                    $method = $accessor->EntityAccessor;
    ...
    public static function getAzureAccessor($member)
    {
        ...
        if (strpos((string) $docComment, '@azure') === false)
        {
            return null;
        }
