...
class Zend_Service_WindowsAzure_Storage
{
	...
	protected function _generateMetadataHeaders($metadata = array())
	{
		...
		foreach ($metadata as $key => $value) {
			if (strpos((string) $value, "\r") !== false || strpos((string) $value, "\n") !== false) {
                            // require_once 'Zend/Service/WindowsAzure/Exception.php';
	...
	protected function _parseMetadataHeaders($headers = array())
	{
		...
		foreach ($headers as $key => $value) {
		    if (substr((string) strtolower((string) $key), 0, 10) == "x-ms-meta-") {
		        $metadata[str_replace((string) "x-ms-meta-", '', strtolower((string) $key))] = $value;
		    }
		}
		return $metadata;
	}
	...
	public function isoDate($timestamp = null)
	{
	    ...
	    $returnValue = str_replace((string) '+00:00', '.0000000Z', @date('c', $timestamp));
