...
class Zend_Service_WindowsAzure_Management_Client
{
	...
	protected function _performRequest(
		...
		// URL encoding
		$path           = self::urlencode((string) $path);
		$queryString    = self::urlencode((string) $queryString);
    ...
    public function listSubscriptionOperations($startTime, $endTime, $objectIdFilter = null, $operationResultFilter = null, $continuationToken = null)
    {
    	...
    	if ($operationResultFilter != '' && !is_null($operationResultFilter)) {
	        $operationResultFilter = strtolower((string) $operationResultFilter);
	...
	protected function _updateInstanceCountInConfiguration($roleName, $instanceCount, $configuration) {
		...
		//$configuration = '<?xml version="1.0">' . substr((string) $configuration, strpos((string) $configuration, '>') + 2);
    ...
    public function getCertificate($serviceName, $algorithm = '', $thumbprint = '')
    {
    	...
    	if (strpos((string) $serviceName, 'https') === false && ($algorithm == '' || is_null($algorithm)) && ($thumbprint == '' || is_null($thumbprint))) {
			// require_once 'Zend/Service/WindowsAzure/Management/Exception.php';
    		throw new Zend_Service_WindowsAzure_Management_Exception('Algorithm and thumbprint should be specified.');
    	}

    	$operationUrl = str_replace((string) $this->getBaseUrl(), '', $serviceName);
    	if (strpos((string) $serviceName, 'https') === false) {
    		$operationUrl = self::OP_HOSTED_SERVICES . '/' . $serviceName . '/certificates/' . $algorithm . '-' . strtoupper((string) $thumbprint);
    	}
    ...
    public function _cleanConfiguration($configuration) {
    	$configuration = str_replace((string) '?<?', '<?', $configuration);
		$configuration = str_replace((string) "\r", "", $configuration);
		$configuration = str_replace((string) "\n", "", $configuration);
