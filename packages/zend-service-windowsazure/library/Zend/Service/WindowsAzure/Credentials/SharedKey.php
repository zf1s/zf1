    ...
    extends Zend_Service_WindowsAzure_Credentials_CredentialsAbstract
{
	...
	public function signRequestHeaders(
		...
		if ($this->_usePathStyleUri) {
			$path = substr((string) $path, strpos((string) $path, '/'));
		}
		...
		if (!is_null($headers)) {
			foreach ($headers as $header => $value) {
				...
				if (substr((string) $header, 0, strlen((string) Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PREFIX_STORAGE_HEADER)) == Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PREFIX_STORAGE_HEADER) {
		...
		if ($queryString !== '') {
		    ...
		    foreach ($queryStringItems as $key => $value) {
		    	$canonicalizedResource .= "\n" . strtolower((string) $key) . ':' . urldecode($value);
		    }
		}

		// Content-Length header
		$contentLength = '';
		if (strtoupper((string) $httpVerb) != Zend_Http_Client::GET
			 && strtoupper((string) $httpVerb) != Zend_Http_Client::DELETE
			 && strtoupper((string) $httpVerb) != Zend_Http_Client::HEAD) {
			$contentLength = 0;

			if (!is_null($rawData)) {
				$contentLength = strlen((string) $rawData);
			}
		}

		// Create string to sign
		$stringToSign   = array();
		$stringToSign[] = strtoupper((string) $httpVerb); 									// VERB
